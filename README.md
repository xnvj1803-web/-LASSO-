...
🌲 深绿（实线）：坐标下降法（Coordinate Descent）
...
请注意 **ADMM（rho=1）** 在高维下的表现：在接近最优解时，L1正则项的不可导性使得软阈值操作：

z = sign(β + u) * max(|β + u| - λ/ρ, 0)
...
1.  **速度最快：Coordinate Descent**
    无论是在低维还是高维，**坐标下降法**都展示了统治级的收敛速度和最低的计算代价。对于 LASSO 这类可分问题，CD 是绝对的首选。

3.  **ADMM 的鲁棒性:**
    ADMM 对参数 $\rho$ 敏感，且在高维下计算昂贵 (**$O(p^3)$**),但它展示了良好的鲁棒性，说明它适合处理约束更复杂的问题，但在纯 LASSO 问题上不如 CD 高效。
